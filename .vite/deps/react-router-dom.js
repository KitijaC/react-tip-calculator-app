...
function getUrlBasedHistory(getLocation, createHref, validateLocation, options) {
  ...
  let history = {
    ...
    encodeLocation(location) {
      let url = createURL2(createPath2(location));
...
function convertRoutesToDataRoutes2(routes, parentPath, allIds) {
  if (parentPath === void 0) {
    parentPath = [];
  }
  if (allIds === void 0) {
  ...
  return routes.map((route, index) => {
    ...
    invariant2(!allIds.has(id), 'Found a route id collision on id "' + id + `".  Route id's must be globally unique within Data Router usages`);
    allIds.add(id);
...
function matchRoutes2(routes, locationArg, basename) {
  ...
  let branches = flattenRoutes2(routes);
...
function flattenRoutes2(routes, branches, parentsMeta, parentPath) {
  if (branches === void 0) {
    branches = [];
  }
  if (parentsMeta === void 0) {
  ...
  routes.forEach((route, index) => {
    ...
    let path = joinPaths2([parentPath, meta.relativePath]);
    let routesMeta = parentsMeta.concat(meta);
    if (route.children && route.children.length > 0) {
      invariant2(
        // Our types know better, but runtime JS may not!
...
function rankRouteBranches2(branches) {
  branches.sort((a, b) => a.score !== b.score ? b.score - a.score : compareIndexes2(a.routesMeta.map((meta) => meta.childrenIndex), b.routesMeta.map((meta) => meta.childrenIndex)));
}
var paramRe2 = /^:\w+$/;
...
function resolveTo2(toArg, routePathnames, locationPathname, isPathRelative) {
  ...
  if (typeof toArg === "string") {
    ...
    to = _extends3({}, toArg);
    invariant2(!to.pathname || !to.pathname.includes("?"), getInvalidPathError2("?", "pathname", "search", to));
    invariant2(!to.pathname || !to.pathname.includes("#"), getInvalidPathError2("#", "pathname", "hash", to));
    invariant2(!to.search || !to.search.includes("#"), getInvalidPathError2("#", "search", "hash", to));
  }
  let isEmptyPath = toArg === "" || to.pathname === "";
  let toPathname = isEmptyPath ? "/" : to.pathname;
  let from;
...
function isRouteErrorResponse2(e) {
  return e instanceof ErrorResponse2;
}
...
function createRouter2(init) {
  ...
  let dataRoutes = convertRoutesToDataRoutes2(init.routes);
  let unlistenHistory = null;
  let subscribers = /* @__PURE__ */ new Set();
  let savedScrollPositions2 = null;
  let getScrollRestorationKey = null;
  let getScrollPosition = null;
  let initialScrollRestored = false;
  let initialMatches = matchRoutes2(dataRoutes, init.history.location, init.basename);
  let initialErrors = null;
  if (initialMatches == null) {
    let {
      matches,
      route,
      error
    } = getNotFoundMatches2(dataRoutes);
    initialMatches = matches;
    initialErrors = {
      [route.id]: error
    };
  ...
  function completeNavigation(location, newState) {
    ...
    let isActionReload = state.actionData != null && state.navigation.formMethod != null && state.navigation.state === "loading" && ((_state$navigation$for = state.navigation.formAction) == null ? void 0 : _state$navigation$for.split("?")[0]) === location.pathname;
    let newLoaderData = newState.loaderData ? {
      loaderData: mergeLoaderData2(state.loaderData, newState.loaderData, newState.matches || [])
    } : {};
    updateState(_extends3({}, isActionReload ? {} : {
      actionData: null
    }, newState, newLoaderData, {
      historyAction: pendingAction,
      location,
  ...
  async function navigate(to, opts) {
    ...
    let {
      ...
      submission,
      error
    } = normalizeNavigateOptions2(to, opts);
    let location = createLocation2(state.location, path, opts && opts.state);
    location = init.history.encodeLocation(location);
    let historyAction = (opts && opts.replace) === true || submission != null ? Action2.Replace : Action2.Push;
    let preventScrollReset = opts && "preventScrollReset" in opts ? opts.preventScrollReset === true : void 0;
    return await startNavigation(historyAction, location, {
      submission,
      // Send through the formData serialization error if we have one so we can
  ...
  async function startNavigation(historyAction, location, opts) {
    ...
    pendingPreventScrollReset = (opts && opts.preventScrollReset) === true;
    let loadingNavigation = opts && opts.overrideNavigation;
    let matches = matchRoutes2(dataRoutes, location, init.basename);
    if (!matches) {
      let {
        matches: notFoundMatches,
        route,
    ...
    if (isHashChangeOnly2(state.location, location)) {
      completeNavigation(location, {
        matches
      });
      return;
    ...
    pendingNavigationController = new AbortController();
    let request = createRequest2(location, pendingNavigationController.signal, opts && opts.submission);
    let pendingActionData;
    let pendingError;
    if (opts && opts.pendingError) {
      pendingError = {
  ...
  async function handleAction(request, location, submission, matches, opts) {
    ...
    let result;
    let actionMatch = getTargetMatch2(matches, location);
    if (!actionMatch.route.action) {
      result = getMethodNotAllowedResult2(location);
    } else {
      result = await callLoaderOrAction2("action", request, actionMatch, matches, router.basename);
      if (request.signal.aborted) {
        return {
          shortCircuited: true
        };
    ...
    if (isRedirectResult2(result)) {
      let redirectNavigation = _extends3({
        state: "loading",
        location: createLocation2(state.location, result.location)
      }, submission);
      await startRedirectNavigation(result, redirectNavigation, opts && opts.replace);
      return {
        shortCircuited: true
  ...
  async function handleLoaders(request, location, matches, overrideNavigation, submission, replace, pendingActionData, pendingError) {
    ...
    let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad2(state, matches, submission, location, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, pendingActionData, pendingError, fetchLoadMatches);
    cancelActiveDeferreds((routeId) => !(matches && matches.some((m) => m.route.id === routeId)) || matchesToLoad && matchesToLoad.some((m) => m.route.id === routeId));
    if (matchesToLoad.length === 0 && revalidatingFetchers.length === 0) {
      completeNavigation(location, {
        matches,
        loaderData: mergeLoaderData2(state.loaderData, {}, matches),
        // Commit pending error if we're short circuiting
        errors: pendingError || null,
        actionData: pendingActionData || null
      });
    ...
    if (redirect3) {
      let redirectNavigation = getLoaderRedirect2(state, redirect3);
      await startRedirectNavigation(redirect3, redirectNavigation, replace);
      return {
        shortCircuited: true
      };
  ...
  function fetch(key, routeId, href, opts) {
    ...
    if (!matches) {
      setFetcherError(key, routeId, new ErrorResponse2(404, "Not Found", null));
      return;
    }
    let {
      path,
      submission
    } = normalizeNavigateOptions2(href, opts, true);
  ...
  async function handleFetcherAction(key, routeId, path, match, requestMatches, submission) {
    ...
    if (!match.route.action) {
      let {
        error
      } = getMethodNotAllowedResult2(path);
      setFetcherError(key, routeId, error);
      return;
    }
    let existingFetcher = state.fetchers.get(key);
    ...
    let abortController = new AbortController();
    let fetchRequest = createRequest2(path, abortController.signal, submission);
    fetchControllers.set(key, abortController);
    let actionResult = await callLoaderOrAction2("action", fetchRequest, match, requestMatches, router.basename);
    if (fetchRequest.signal.aborted) {
      if (fetchControllers.get(key) === abortController) {
        fetchControllers.delete(key);
      }
    ...
    if (isRedirectResult2(actionResult)) {
      ...
      let redirectNavigation = _extends3({
        state: "loading",
        location: createLocation2(state.location, actionResult.location)
      }, submission);
      await startRedirectNavigation(actionResult, redirectNavigation);
      return;
    }
    ...
    if (isDeferredResult2(actionResult)) {
      invariant2(false, "defer() is not supported in actions");
    }
    let nextLocation = state.navigation.location || state.location;
    let revalidationRequest = createRequest2(nextLocation, abortController.signal);
    let matches = state.navigation.state !== "idle" ? matchRoutes2(dataRoutes, state.navigation.location, init.basename) : state.matches;
    ...
    if (state.navigation.state === "loading" && loadId > pendingNavigationLoadId) {
      ...
      updateState(_extends3({
        errors,
        loaderData: mergeLoaderData2(state.loaderData, loaderData, matches)
      }, didAbortFetchLoads ? {
        fetchers: new Map(state.fetchers)
      } : {}));
      isRevalidationRequired = false;
  ...
  async function handleFetcherLoader(key, routeId, path, match, matches) {
    ...
    let abortController = new AbortController();
    let fetchRequest = createRequest2(path, abortController.signal);
    fetchControllers.set(key, abortController);
    let result = await callLoaderOrAction2("loader", fetchRequest, match, matches, router.basename);
    if (isDeferredResult2(result)) {
    ...
    if (isRedirectResult2(result)) {
      let redirectNavigation = getLoaderRedirect2(state, result);
      await startRedirectNavigation(result, redirectNavigation);
      return;
    }
    if (isErrorResult2(result)) {
    ...
    invariant2(!isDeferredResult2(result), "Unhandled fetcher deferred data");
    let doneFetcher = {
      state: "idle",
      data: result.data,
      formMethod: void 0,
  ...
  async function startRedirectNavigation(redirect3, navigation, replace) {
    ...
    invariant2(navigation.location, "Expected a location on the redirect navigation");
    pendingNavigationController = null;
  ...
  async function callLoadersAndMaybeResolveData(currentMatches, matches, matchesToLoad, fetchersToLoad, request) {
    let results = await Promise.all([...matchesToLoad.map((match) => callLoaderOrAction2("loader", request, match, matches, router.basename)), ...fetchersToLoad.map((_ref8) => {
      let [, href, match, fetchMatches] = _ref8;
      return callLoaderOrAction2("loader", createRequest2(href, request.signal), match, fetchMatches, router.basename);
    })]);
    let loaderResults = results.slice(0, matchesToLoad.length);
    let fetcherResults = results.slice(matchesToLoad.length);
    await Promise.all([resolveDeferredResults2(currentMatches, matchesToLoad, loaderResults, request.signal, false, state.loaderData), resolveDeferredResults2(currentMatches, fetchersToLoad.map((_ref9) => {
      let [, , match] = _ref9;
      return match;
    }), fetcherResults, request.signal, true)]);
    return {
  ...
  function saveScrollPosition(location, matches) {
    if (savedScrollPositions2 && getScrollRestorationKey && getScrollPosition) {
      let userMatches = matches.map((m) => createUseMatchesMatch2(m, state.loaderData));
      let key = getScrollRestorationKey(location, userMatches) || location.key;
      savedScrollPositions2[key] = getScrollPosition();
    }
  }
...
var validActionMethods2 = /* @__PURE__ */ new Set(["POST", "PUT", "PATCH", "DELETE"]);
var validRequestMethods2 = /* @__PURE__ */ new Set(["GET", "HEAD", ...validActionMethods2]);
function normalizeNavigateOptions2(to, opts, isFetcher) {
  ...
  try {
    let searchParams = convertFormDataToSearchParams2(opts.formData);
    ...
    return {
      path,
      error: new ErrorResponse2(400, "Bad Request", "Cannot submit binary form data using GET")
    };
  }
  return {
...
function getLoaderRedirect2(state, redirect3) {
  let {
    formMethod,
    formAction,
    formEncType,
  ...
  let navigation = {
    state: "loading",
    location: createLocation2(state.location, redirect3.location),
    formMethod: formMethod || void 0,
    formAction: formAction || void 0,
    formEncType: formEncType || void 0,
    formData: formData || void 0
...
function getMatchesToLoad2(state, matches, submission, location, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, pendingActionData, pendingError, fetchLoadMatches) {
  let actionResult = pendingError ? Object.values(pendingError)[0] : pendingActionData ? Object.values(pendingActionData)[0] : null;
  let boundaryId = pendingError ? Object.keys(pendingError)[0] : void 0;
  let boundaryMatches = getLoaderMatchesUntilBoundary2(matches, boundaryId);
  let navigationMatches = boundaryMatches.filter((match, index) => match.route.loader != null && (isNewLoader2(state.loaderData, state.matches[index], match) || // If this route had a pending deferred cancelled it must be revalidated
  cancelledDeferredRoutes.some((id) => id === match.route.id) || shouldRevalidateLoader2(state.location, state.matches[index], submission, location, match, isRevalidationRequired, actionResult)));
  let revalidatingFetchers = [];
  fetchLoadMatches && fetchLoadMatches.forEach((_ref10, key) => {
    let [href, match, fetchMatches] = _ref10;
    if (cancelledFetcherLoads.includes(key)) {
      ...
      let shouldRevalidate = shouldRevalidateLoader2(href, match, submission, href, match, isRevalidationRequired, actionResult);
      if (shouldRevalidate) {
        revalidatingFetchers.push([key, href, match, fetchMatches]);
      }
    }
...
function shouldRevalidateLoader2(currentLocation, currentMatch, submission, location, match, isRevalidationRequired, actionResult) {
  let currentUrl = createURL2(currentLocation);
  let currentParams = currentMatch.params;
  let nextUrl = createURL2(location);
  let nextParams = match.params;
  let defaultShouldRevalidate = isNewRouteInstance2(currentMatch, match) || // Clicked the same link, resubmitted a GET form
  currentUrl.toString() === nextUrl.toString() || // Search params affect all loaders
  currentUrl.search !== nextUrl.search || // Forced revalidation due to submission, useRevalidate, or X-Remix-Revalidate
  isRevalidationRequired;
  if (match.route.shouldRevalidate) {
...
async function callLoaderOrAction2(type, request, match, matches, basename, isStaticRequest, isRouteRequest) {
  if (isStaticRequest === void 0) {
    isStaticRequest = false;
  }
  if (isRouteRequest === void 0) {
  ...
  try {
    let handler = match.route[type];
    invariant2(handler, "Could not find the " + type + ' to run on the "' + match.route.id + '" route');
    result = await Promise.race([handler({
      request,
      params: match.params
    }), abortPromise]);
  ...
  if (result instanceof Response) {
    ...
    if (status >= 300 && status <= 399) {
      ...
      let activeMatches = matches.slice(0, matches.indexOf(match) + 1);
      let routePathnames = getPathContributingMatches2(activeMatches).map((match2) => match2.pathnameBase);
      let requestPath = createURL2(request.url).pathname;
      let resolvedLocation = resolveTo2(location, routePathnames, requestPath);
      invariant2(createPath2(resolvedLocation), "Unable to resolve redirect location: " + result.headers.get("Location"));
      if (basename) {
        let path = resolvedLocation.pathname;
  ...
  if (result instanceof DeferredData2) {
    return {
...
function createRequest2(location, signal, submission) {
  let url = createURL2(stripHashFromPath2(location)).toString();
  let init = {
    signal
  };
  if (submission) {
    ...
    init.method = formMethod.toUpperCase();
    init.body = formEncType === "application/x-www-form-urlencoded" ? convertFormDataToSearchParams2(formData) : formData;
  }
  return new Request(url, init);
}
...
function processRouteLoaderData2(matches, matchesToLoad, results, pendingError, activeDeferreds) {
  ...
  results.forEach((result, index) => {
    let id = matchesToLoad[index].route.id;
    invariant2(!isRedirectResult2(result), "Cannot handle redirect results in processLoaderData");
    if (isErrorResult2(result)) {
      let boundaryMatch = findNearestBoundary2(matches, id);
      let error = result.error;
      if (pendingError) {
        error = Object.values(pendingError)[0];
        pendingError = void 0;
...
function mergeLoaderData2(loaderData, newLoaderData, matches) {
...
function getShortCircuitMatches2(routes, status, statusText) {
  let route = routes.find((r) => r.index || !r.path || r.path === "/") || {
    id: "__shim-" + status + "-route__"
  };
  return {
    ...
    route,
    error: new ErrorResponse2(status, statusText, null)
  };
}
function getNotFoundMatches2(routes) {
  return getShortCircuitMatches2(routes, 404, "Not Found");
}
function getMethodNotAllowedResult2(path) {
  let href = typeof path === "string" ? path : createPath2(path);
  console.warn("You're trying to submit to a route that does not have an action.  To fix this, please add an `action` function to the route for " + ("[" + href + "]"));
  return {
    type: ResultType2.error,
    error: new ErrorResponse2(405, "Method Not Allowed", "")
  };
}
...
async function resolveDeferredResults2(currentMatches, matchesToLoad, results, signal, isFetcher, currentLoaderData) {
  for (let index = 0; index < results.length; index++) {
    ...
    let match = matchesToLoad[index];
    let currentMatch = currentMatches.find((m) => m.route.id === match.route.id);
    let isRevalidatingLoader = currentMatch != null && !isNewRouteInstance2(currentMatch, match) && (currentLoaderData && currentLoaderData[match.route.id]) !== void 0;
...
var Link = React2.forwardRef(function LinkWithRef(_ref4, ref) {
  ...
  return (
    ...
    React2.createElement("a", _extends4({}, rest, {
      href,
...
var NavLink = React2.forwardRef(function NavLinkWithRef(_ref5, ref) {
  ...
  return React2.createElement(Link, _extends4({}, rest, {
...
var Form = React2.forwardRef((props, ref) => {
  return React2.createElement(FormImpl, _extends4({}, props, {
    ref
  }));
});
if (true) {
...
var FormImpl = React2.forwardRef((_ref6, forwardedRef) => {
  ...
  return React2.createElement("form", _extends4({
    ref: forwardedRef,
    method: formMethod,
    action: formAction,
...
function useFetcher() {
  ...
  let [Form2] = React2.useState(() => {
    !routeId ? true ? invariant2(false, "No routeId available for fetcher.Form()") : invariant2(false) : void 0;
    return createFetcherForm(fetcherKey, routeId);
  });
  let [load] = React2.useState(() => (href) => {
    !router ? true ? invariant2(false, "No router available for fetcher.load()") : invariant2(false) : void 0;
    !routeId ? true ? invariant2(false, "No routeId available for fetcher.load()") : invariant2(false) : void 0;
    router.fetch(fetcherKey, routeId, href);
  });
  let submit = useSubmitImpl(fetcherKey, routeId);
  let fetcher = router.getFetcher(fetcherKey);
  let fetcherWithComponents = React2.useMemo(() => _extends4({
    Form: Form2,
    submit,
    load
